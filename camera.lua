function loadCamera(maxX, maxY, dotToUse)
  cameraX = 99--dotToUse.x - (1/5*love.graphics.getWidth()/30)
...
function updateCamera(dotToUse)
  if (dotToUse.x < (cameraX + 1/5*love.graphics.getWidth()/30)) and (0 <= dotToUse.x) then
  ...
  if (dotToUse.y < (cameraY + 1/5*love.graphics.getHeight()/30)) and (0 <= dotToUse.y) then
    cameraY = dotToUse.y - (1/5*love.graphics.getHeight()/30)
